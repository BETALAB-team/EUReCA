...
class RSMDef(object):
    ...
    def __init__(self,lat,lon,GMT,height,T_init,P_init,parameter,z_meso_path = None):
    ...
    def VDM(self,temp, pres, wind,sens,parameter,dt = 3600):

        self.tempProf[0] = temp    # Lower boundary condition
        # compute pressure profile
        for iz in reversed(list(range(self.nzref))[1:]):
           self.presProf[iz-1] = (math.pow(self.presProf[iz],parameter.r/parameter.cp) + \
               parameter.g/parameter.cp*(math.pow(pres,parameter.r/parameter.cp)) * \
        ...
        for iz in range(self.nzref):
            ...
            (self.presProf[iz]/pres)**(parameter.r/parameter.cp)
        ...
        cd,ustarRur = self.DiffusionCoefficient(self.densityProfC[0], \
            ...
            self.tempProf[0], sens, self.nzref, wind, \
            self.tempProf, parameter)


        # solve diffusion equation
        self.tempProf = self.DiffusionEquation(self.nzref,dt,\
