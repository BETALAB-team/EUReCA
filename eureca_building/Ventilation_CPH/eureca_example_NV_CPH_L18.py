...
def simulation(
    ...
    wall_West = Surface(
        "Wall West",
        vertices=((0., 0., 6.6), (0., 0., 9.3), (0., 8.6, 9.3), (0., 8.6, 6.6)),  # The third coordinate takes into account the flat floor number (each floor of 3.3 m - external dimensions)
        wwr=0.40,
    ...
    wall_South = Surface(
        "Wall South",
        vertices=((0., 0., 6.6), (12.3, 0., 6.6), (12.3, 0., 9.3), (0., 0., 9.3)),  # The third coordinate takes into account the flat floor number (each floor of 3.3 m - external dimensions)
        wwr=0.33,
        surface_type="ExtWall",
    ...
    bd = Building("BdL18", thermal_zones_list=[tz1], model = "1C")
...
#########################################################
# Measure loading
measure = pd.read_csv("C:\\Users\\gecky\\OneDrive - Università degli Studi di Padova\\PhD_directory\\AAU material\\DataComfortCooling\\Collected_data\\IC-Meter-QRA6474C62-Indoor-Minutes-01-Jun-2023-29-Oct-2023.csv",
                      skiprows = 0, header = 1, delimiter = ';', decimal = ',', index_col = 0, parse_dates = True)
measure.drop(["DATE (EUROPE/COPENHAGEN)", "TIME (EUROPE/COPENHAGEN)"], axis = 1, inplace = True)
