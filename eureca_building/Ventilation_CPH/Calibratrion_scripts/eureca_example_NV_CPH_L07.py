...
def simulation(
    ...
    ext_wall_East = Construction.from_U_value("EastExtWall", 0.19, weight_class = "Medium", construction_type = "ExtWall")  # At the moment, I left "medium" weight class
    ...
    wall_East = Surface(
        "Wall East",
        vertices=((12.3, 2.7, 6.), (12.3, 6., 6.), (12.3, 6., 8.7), (12.3, 2.7, 8.7)),  # The third coordinate takes into account the flat floor number (each floor of 3.0 m - external dimensions)
        wwr=0.45,
    ...
    wall_West = Surface(
        "Wall West",
        vertices=((0., 2.3, 6.), (0., 2.3, 8.7), (0., 6., 8.7), (0., 6., 6.)),  # The third coordinate takes into account the flat floor number (each floor of 3.0 m - external dimensions)
        wwr=0.27,
        surface_type="ExtWall",
    ...
    int_part = SurfaceInternalMass(
        "IntPart",
        area=92,     # Area is doubled since partitions inside the occupied zone present two surfaces
        surface_type="IntWall",
    ...
    int_floor = SurfaceInternalMass(
        "IntFloor",
        area=54,  # Floor area based the average value between internal and external dimensions
    ...
    int_ceil = SurfaceInternalMass(
        "IntCeil",
        area=54,  # Ceiling area based the average value between internal and external dimensions
    ...
    tz1 = ThermalZone(
        name="Zone 1",
        surface_list=[wall_East, wall_West, int_wall, int_part, int_floor, int_ceil],
        net_floor_area=48, # Net floor area based on internal dimensions
        volume=48*2.7)
    ...
    bd = Building("BdL07", thermal_zones_list=[tz1], model = "1C")
...
#########################################################
# Measure loading
measure = pd.read_csv("C:\\Users\\gecky\\OneDrive - Università degli Studi di Padova\\PhD_directory\\AAU material\\DataComfortCooling\\1_Collected_data\\IC-Meter-QR728C79F8-Indoor-Minutes-01-Jun-2023-29-Oct-2023.csv",
                      skiprows = 0, header = 1, delimiter = ';', decimal = ',', index_col = 0, parse_dates = True)
measure.drop(["DATE (EUROPE/COPENHAGEN)", "TIME (EUROPE/COPENHAGEN)"], axis = 1, inplace = True)
...
output_file_name = "Calibration_results_L07"
