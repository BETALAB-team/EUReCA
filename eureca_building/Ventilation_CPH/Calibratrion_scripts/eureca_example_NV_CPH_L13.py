...
def simulation(
    ...
    wall_North = Surface(
        "Wall North",
        vertices=((0., 12.3, 12.), (0., 12.3, 14.7), (7.3, 12.3, 14.7), (7.3, 12.3, 12.)),  # The third coordinate takes into account the flat floor number (each floor of 3.0 m - external dimensions)
        wwr=0.36,
    ...
    wall_South = Surface(
        "Wall South",
        vertices=((0., 0., 12.), (7.3, 0., 12.), (7.3, 0., 14.7), (0., 0., 14.7)),  # The third coordinate takes into account the flat floor number (each floor of 3.0 m - external dimensions)
        wwr=0.39,
        surface_type="ExtWall",
    ...
    roof_N = Surface(
        "Roof",
        vertices=((0., 12.3, 14.7), (0., 10.7, 14.7), (1.0, 10.7, 14.7), (1.0, 8.5, 14.7), (1.5, 8.5, 14.7), (7.3, 8.5, 14.7), (7.3, 12.3, 14.7)),
        wwr=0,
        surface_type="Roof",
    ...
    roof_S = Surface(
        "Roof",
        vertices=((0., 0., 14.7), (7.3, 0., 14.7), (7.3, 4.2, 14.7), (0., 4.2, 14.7)),
        wwr=0,
        surface_type="Roof",
        construction=roof_constr,
        window=window,
    ...
    int_part = SurfaceInternalMass(
        "IntPart",
        area=140,     # Area is doubled since partitions inside the occupied zone present two surfaces
        surface_type="IntWall",
    ...
    int_floor = SurfaceInternalMass(
        "IntFloor",
        area=81,  # Floor area based the average value between internal and external dimensions
    ...
    int_ceil = SurfaceInternalMass(
        "IntCeil",
        area=25,  # Ceiling area based the average value between internal and external dimensions
    ...
    tz1 = ThermalZone(
        name="Zone 1",
        surface_list=[wall_North, wall_South, roof_N, roof_S, int_wall, int_part, int_floor, int_ceil],
        net_floor_area=75, # Net floor area based on internal dimensions
        volume=75*2.7)
    ...
    bd = Building("BdL13", thermal_zones_list=[tz1], model = "1C")
...
#########################################################
# Measure loading
measure = pd.read_csv("C:\\Users\\gecky\\OneDrive - Università degli Studi di Padova\\PhD_directory\\AAU material\\DataComfortCooling\\1_Collected_data\\IC-Meter-QR5B3B057D-Indoor-Minutes-01-Jun-2023-29-Oct-2023.csv",
                      skiprows = 0, header = 1, delimiter = ';', decimal = ',', index_col = 0, parse_dates = True)
measure.drop(["DATE (EUROPE/COPENHAGEN)", "TIME (EUROPE/COPENHAGEN)"], axis = 1, inplace = True)
...
output_file_name = "Calibration_results_L13"
...
result_table.to_csv(os.path.join('.', 'Results', 'CalibRes', f"{output_file_name}.csv"), float_format='%.4f', index = True, sep =";")