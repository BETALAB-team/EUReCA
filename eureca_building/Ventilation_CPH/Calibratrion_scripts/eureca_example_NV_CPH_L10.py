...
def simulation(
    ...
    wall_North = Surface(
        "Wall North",
        vertices=((0., 12.3, 6.), (0., 12.3, 8.7), (5.6, 12.3, 8.7), (5.6, 12.3, 6.)),  # The third coordinate takes into account the flat floor number (each floor of 3.0 m - external dimensions)
        wwr=0.36,
    ...
    wall_South = Surface(
        "Wall South",
        vertices=((0., 0., 6.), (5.6, 0., 6.), (5.6, 0., 8.7), (0., 0., 8.7)),  # The third coordinate takes into account the flat floor number (each floor of 3.0 m - external dimensions)
        wwr=0.33,
        surface_type="ExtWall",
    ...
    int_part = SurfaceInternalMass(
        "IntPart",
        area=116,     # Area is doubled since partitions inside the occupied zone present two surfaces
        surface_type="IntWall",
    ...
    int_floor = SurfaceInternalMass(
        "IntFloor",
        area=69,  # Floor area based the average value between internal and external dimensions
    ...
    int_ceil = SurfaceInternalMass(
        "IntCeil",
        area=69,  # Ceiling area based the average value between internal and external dimensions
    ...
    tz1 = ThermalZone(
        name="Zone 1",
        surface_list=[wall_North, wall_South, int_wall, int_part, int_floor, int_ceil],
        net_floor_area=60, # Net floor area based on internal dimensions
        volume=60*2.7)
    ...
    bd = Building("BdL10", thermal_zones_list=[tz1], model = "1C")
...
#########################################################
# Measure loading
measure = pd.read_csv("C:\\Users\\gecky\\OneDrive - Università degli Studi di Padova\\PhD_directory\\AAU material\\DataComfortCooling\\1_Collected_data\\IC-Meter-QREC2713FE-Indoor-Minutes-01-Jun-2023-29-Oct-2023.csv",
                      skiprows = 0, header = 1, delimiter = ';', decimal = ',', index_col = 0, parse_dates = True)
measure.drop(["DATE (EUROPE/COPENHAGEN)", "TIME (EUROPE/COPENHAGEN)"], axis = 1, inplace = True)
...
output_file_name = "Calibration_results_L10"
