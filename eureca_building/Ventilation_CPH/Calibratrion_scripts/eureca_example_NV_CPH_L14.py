...
def simulation(
    ...
    int_part = SurfaceInternalMass(
        "IntPart",
        area=166,     # Area is doubled since partitions inside the occupied zone present two surfaces
        surface_type="IntWall",
    ...
    tz1 = ThermalZone(
        name="Zone 1",
        surface_list=[wall_North, wall_East, wall_West, roof, int_wall, int_part, int_floor],
        net_floor_area=85, # Net floor area based on internal dimensions
        volume=85*2.7)
    ...
    bd = Building("BdL14", thermal_zones_list=[tz1], model = "1C")
